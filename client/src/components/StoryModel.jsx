...
function StoryModel({ setShowModal, fetchStories }) {
    const bgColor = [
        "#FF007F", "#FF8C00", "#7B2CBF", "#00B4D8", // Vibrant
        "#708090", "#B2AC88", "#DCAE96", // Muted
        "#FF6B6B", "#4834D4", "#2ECC71", "#F1C40F", "#6D214F", // Flat UI
    ]
        ...
        if (file) {
            if (file.type.startsWith('video')) {
                if (file.size > MAX_VIDEO_SIZE_MB * 1024 * 1024) {
                    toast.error(`Video size is more than ${MAX_VIDEO_SIZE_MB}mb.`)
                    setMedia(null)
                    setPreviewUrl(null)
                    return
                }
                ...
                video.onloadedmetadata = () => {
                    ...
                    if (video.duration > MAX_VIDEO_DURATION) {
                        toast.error("Video duration connot exceed 1 min.")
                        setMedia(null)
                        setPreviewUrl(null)
                    }
                    else {
        ...
        <div className='fixed inset-0 z-110 min-h-screen bg-black/80 backdrop-blur  text-white flex items-center justify-center p-4'>
            <div className='w-full max-w-md'>
                ...
                <div className='rounded-lg h-96 flex items-center justify-center relative' style={{ backgroundColor: background }}>
                    {
                        mode === 'text' && (
                            <textarea className='bg-transparent text-white w-full h-full p-6 text-lg resize-none focus:outline-none' placeholder='What is in your mind?' onChange={(e) => setText(e.target.value)} value={text} />
                        )
                    }
                    {
